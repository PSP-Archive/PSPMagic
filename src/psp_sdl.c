...
#include "psp_danzeff.h"
  ...
  SDL_Surface *board_silver;
  SDL_Surface *board_gold;
  SDL_Surface *back_silver;
  SDL_Surface *back_gold;
...
{
  back_silver = IMG_Load("./back_silver.png");
  back_gold   = IMG_Load("./back_gold.png");
}

void
psp_sdl_load_board()
{
  board_silver = IMG_Load("./board_silver.png");
  board_gold   = IMG_Load("./board_gold.png");
}

void
psp_sdl_blit_background()
{
  if (MAGIC.magic_theme == 0)  back2_surface = back_silver;
  else                         back2_surface = back_gold;
...
psp_sdl_blit_board()
{
  if (MAGIC.magic_theme == 0)  back3_surface = board_silver;
  else                         back3_surface = board_gold;
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"./scr/screenshot_%d.png", MAGIC.psp_screenshot_id++);
  if (MAGIC.psp_screenshot_id >= 10) MAGIC.psp_screenshot_id = 0;
