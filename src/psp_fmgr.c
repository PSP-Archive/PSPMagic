...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if (!strcasecmp(pszExt, ".mag")) {
      return PSP_FMGR_FORMAT_MAG;
    }
  }
  return 0;
}
...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back2_print(200, 170, "Delete a file", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_back2_print(200, 180, "press X to confirm !", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
static char user_filedir_mag[PSP_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    first = 0;
    getcwd(user_filedir_mag, PSP_FMGR_MAX_NAME);
    strcat(user_filedir_mag, "/mag/");
  }
  user_filedir = user_filedir_mag;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == PSP_FMGR_FORMAT_MAG) /* Mag */ error = psp_magic_load(user_filename);
    }
